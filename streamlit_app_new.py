...
class UserAuthSystem:
    ...
    def show_login_page():
        ...
        with col1:
            if st.button("👤 Demo User", type="primary", use_container_width=True):
                UserAuthSystem._authenticate_user("demo@nxtrix.com", "solo")
                st.rerun()
        
        with col2:
            if st.button("👥 Team Demo", type="primary", use_container_width=True):
                UserAuthSystem._authenticate_user("team@nxtrix.com", "team")
                st.rerun()
        
        with col3:
            if st.button("🏢 Business Demo", type="primary", use_container_width=True):
    ...
    def show_user_menu():
        ...
        if st.session_state.get('user_authenticated', False):
            ...
            *Note: Founder pricing available on landing page*
            """)
        ...
        if st.form_submit_button("🔍 Analyze Deal", type="primary"):
            if address and purchase_price > 0:
                ...
                if ai_score >= 80:
                    ...
                    st.info("💡 Good deal with moderate returns")
                else:
                    st.warning("⚠️ Consider negotiating or looking for better opportunities")
            else:
...
def show_deal_database():
    ...
    # Add new deal button
    if st.button("➕ Add New Deal"):
        navigate_to_page("🏘️ Deal Analysis")
...
def show_investor_portal():
    ...
    if not tier_system.check_feature_access('client_management'):
        st.warning("⚠️ **Investor Portal requires Team tier or higher**")
    ...
    st.subheader("👥 Current Investors")
    st.dataframe(investors, use_container_width=True)
    
    st.subheader("📊 Investor Performance")
...
def show_advanced_financial_modeling():
    ...
    if not tier_system.check_feature_access('advanced_analytics'):
        st.warning("⚠️ **Advanced Financial Modeling requires Team tier or higher**")
...
def show_enhanced_deal_management():
    st.header("🏘️📋 Enhanced Deal Manager")
...
def main():
    ...
    elif page == "🏘️📋 Enhanced Deal Manager":
    ...
    elif page == "💼 Enhanced CRM" and ENHANCED_CRM_AVAILABLE:
        show_enhanced_crm()
    else:
