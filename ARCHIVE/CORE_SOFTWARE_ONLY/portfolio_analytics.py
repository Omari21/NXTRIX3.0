...
def show_portfolio_analytics():
    """Portfolio Analytics main display function for streamlit integration"""
    ...
    try:
        ...
        if not deals:
            st.info("📊 No portfolio data available yet. Add some deals to see analytics!")
            return
        ...
        # Display dashboard
        create_portfolio_metrics_dashboard(metrics)
        st.divider()
        create_portfolio_performance_chart(performances)
        ...
        # Demo metrics
        st.metric("Total Portfolio Value", "$1,250,000")
        st.metric("Total ROI", "18.5%")
        st.metric("Annual Return", "12.3%")